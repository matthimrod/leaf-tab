...
class RiseDispensary(Dispensary):
    def __init__(self, location_name: str, store_id: int):
        super().__init__()
        ...
        with requests.Session() as session:
            ...
            while inventory_url.query_items['page'] <= total_pages:  # type: ignore
                ...
                inventory_url.query_items['page'] += 1  # type: ignore
                        ...
                        for weight_name, price_name, special_price_name in \
                            ...
                            if item[price_name]:
                                row = Product(id=item['product_id'],
                                              ...
                                              cannabinoids={y['compound_name']: float(y['value']) / 100
                                ...
                                if item[special_price_name]:
                                    row.sale_price = float(item[special_price_name]['discount_price'])
                                    if item[special_price_name]['discount_type'] == 'percent':
                                        row.sale_type = f"{item[special_price_name]['discount_percent']}% off"
                                    elif item[special_price_name]['discount_type'] == 'target_price':
                                        row.sale_type = f"${item[special_price_name]['discount_price']} sale"
